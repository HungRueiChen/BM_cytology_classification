...
df = pd.read_csv('../Bone_marrow_cytology_tiles/Classification_20230827.csv')
...
log_dir = Path('../BM_cytology_tile_select/0523_VGG16_20e_0001_then_180e_00001_SGD')
...
for slide_id in slide_list:
    ...
    slide_dir = Path(f'../Bone_marrow_cytology_tiles/{slide_id}_tiles')
    ...
    end_time = time.time()
    if end_time - start_time > 25200:
        command = f'cd {prob_csv_dir} && sbatch -A yu_ky98 --job-name=prob -p gpu --gres=gpu:1 -t 0-07:58 -n 1 --mem 8G 0codes/setup.sh python predict_probs_V3.py'
