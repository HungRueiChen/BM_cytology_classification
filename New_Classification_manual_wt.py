'''
Xception, MobileNetV3Large, InceptionResNetv2, NASNetMobile, ConvNeXtTiny, EfficientNetV2B3
'''
# NASNetLarge, VGG19, ResNet101
import numpy as np
...
class_weight = {0: 2.959459459, 1: 0.73, 2: 2.4333333, 3: 0.356097561, 4: 0.966887417}
print(class_weight)
...
if args.mode == 'initiate':
    ...
    h5_epoch_list = [int(x.stem[12:]) for x in log_dir.glob('*.h5') if 'checkpoint' in x.stem]
    tf_latest_epoch = max(tf_epoch_list) if tf_epoch_list else 0
    h5_latest_epoch = max(h5_epoch_list) if h5_epoch_list else 0
    latest_epoch = max(tf_latest_epoch, h5_latest_epoch)
    ...
    if tf_latest_epoch >= h5_latest_epoch:
        print(f'Loading model: e{latest_epoch}')
        model = load_model(log_dir / f'checkpoint_e{latest_epoch}', compile = True)
    else:
        print(f'Loading model: e{latest_epoch}.h5')
        model = load_model(log_dir / f'checkpoint_e{latest_epoch}.h5', compile = True)
...
if latest_epoch < (args.freeze_epochs + args.fine_tune_epochs):
    ...
    command = f'cd {log_dir} && sbatch -A yu_ky98 --job-name={job_name} -p gpu --gres=gpu:1 -t 0-11:58 -n 1 --mem 12G ../../0codes/setup.sh python New_Classification_manual_wt.py {args.directory} --mode resume --architecture {args.architecture} --optimizer {args.optimizer} --batch_size {args.batch_size} --freeze_lr {args.freeze_lr} --freeze_epochs {args.freeze_epochs} --fine_tune_epochs {args.fine_tune_epochs}'
else:
